...
Server.getDefaults = function() {
  return { 'title': 'mqtt - tp-link bridge' };
}
...
Server.startServer = function(server) {
  const tplinkApi = new tplinkClient();
  ...
  mqttClient.on('message', (topic, message) => {
    if (deviceMap[topic]) {
      ...
      const tplink = tplinkApi.getDevice({host: device.ip}).then((device) => {
        if (message == 'on') {
        ...
        pushActivity('tp-link request failed:' +  error);
      });
    } else {
